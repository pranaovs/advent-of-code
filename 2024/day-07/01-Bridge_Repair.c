...
void int_to_binary_str(int n, int digits, char *binary_str) {
  for (int i = digits - 1; i >= 0; i--) {
    binary_str[digits - 1 - i] = (n & (1 << i)) ? '1' : '0';
  }
}
...
int main(int argc, char *argv[]) {
  ...
  for (int i = 0; getline(&line, &n, f) != -1; i++) {
    ...
    for (int k = 0; k < (pow(2, nums_count - 1)); k++) {

      int_to_binary_str(k, nums_count - 1, operations);
