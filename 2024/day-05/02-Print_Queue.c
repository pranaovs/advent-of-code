// Advent of code 2024 Day-02 Part-02
...
// Function to correct an invalid queue
...
// Function to insert an integer before another integer in a queue
void insert_before(int *queue, int queue_count, int pos0, int pos1) {
  int temp = queue[pos0];
  ...
  for (int j = 0; j < queue_count; j++) {
    ...
    for (int i = 0; i < rules_count; i++) {
      ...
      if (pos0 != -1 && pos1 != -1) {
        if (pos0 > pos1) {
          valid = false;
          insert_before(queue, queue_count, pos0, pos1);
        }
      }
    }
  }
...
int main(int argc, char *argv[]) {
  ...
  for (int line_no = 0; fgets(line, sizeof(line), f); line_no++) {
    ...
    if (program_state == RULES) {
      ...
      // Validate queue against rules and correct if necessary (returns false)
      bool ans = validate_queue(queue, queue_count, rules, rules_count);
      if (!ans) {
        printf("corrected queue: ");
        for (int i = 0; i < queue_count; i++) {
          printf("%d ", queue[i]);
        }
        printf("\n");
